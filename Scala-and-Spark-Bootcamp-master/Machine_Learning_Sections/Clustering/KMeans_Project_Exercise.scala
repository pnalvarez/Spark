...
val dataset = spark.read.option("header","true").option("inferSchema","true").format("csv").load("Machine_Learning_Sections/Clustering/Wholesale customers data.csv")
...
val training = assembler.transform(feature_data).select("features")
