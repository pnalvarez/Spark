...
val advertising = spark.read.option("header","true").option("inferSchema","true").format("csv").load("Machine_Learning_Sections/Classification/advertising.csv")
...
val columns = advertising.columns
val row = advertising.head(1)(0)
println("\n")
...
val newdf = advertising.select(advertising("Clicked on Ad").as("label"), $"Daily Time Spent on Site",$"Age",$"Area Income",$"Daily Internet Usage",$"Timestamp",$"Male").withColumn("Hour", hour(advertising("Timestamp")))
newdf.na.drop()
...
val Array(training,test) = newdf.randomSplit(Array(0.7,0.3),seed = 12345)
...
val finalresult = results.select($"prediction",$"label").as[(Double,Double)].rdd
